...
function App() {
  ...
  const saveCandidate = async (candidate: Candidate, resumeText: string, jobDescription: string) => {
    ...
    if (supabase && session.organizationId && session.userId) {
      const { data, error } = await supabase.from('candidates').insert({ organization_id: session.organizationId, job_id: candidate.jobId, full_name: candidate.name, email: candidate.email, phone: candidate.phone, location: candidate.location, linkedin_url: candidate.linkedin, stage: candidate.stage, created_by: session.userId }).select('id').single()
      if (error) { setNotice(`Candidate could not be saved: ${error.message}`); return }
      saved = { ...candidate, id: data.id }
      if (resumeText.trim()) {
