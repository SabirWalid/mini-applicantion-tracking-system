...
Deno.serve(async request => {
  ...
  const { email, fullName, role, organizationName } = await request.json()
  ...
  if (organizationName) await adminClient.from('organizations').update({ name: organizationName }).eq('id', organizationId)
